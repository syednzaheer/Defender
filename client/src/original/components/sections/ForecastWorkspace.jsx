...
const ForecastWorkspace = ({ autoRunDemo = false, onForecastResult }) => {
  ...
  const runForecastExecution = async (overrideMode = activeMode, overrideFile = selectedFile) => {
    ...
    try {
      ...
      setErrorMsg(`${err.message || 'Forecast execution failed.'} Showing the local validated fallback.`);
  ...
  const handleFileSelect = (e) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      setErrorMsg('CSV exceeds the 50 MB safety limit.');
      return;
    }
      ...
      <div className="forecast-header" style={{ marginBottom: '32px' }}>
        <div className="forecast-header-copy">
          <div className="forecast-meta-row" style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-muted)' }}>OFFLINE INFERENCE ENGINE // SIH 26153</span>
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            >
              ...
              <option value="Trained LSTM demo artifact">Trained LSTM demo artifact (Synthetic)</option>
            </select>
          </div>
