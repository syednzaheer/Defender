...
beats a classifier" slide, and it's the exact number the world model needs
...
| `clean_features_SAMPLE.csv` / `normalized_features_SAMPLE.csv` | **First 2,000 rows only** — for the modeling owner to check the schema before running the pipeline himself. The real files are 2.49M rows / ~280MB and ~880MB, too large to ship in this zip. |

## ⚠️ You (or the modeling owner) need to regenerate the full files locally
...
file, which would have forced the modeling owner to reimplement the scaling step
...
`data_loader.py`'s label distribution output) before the modeling stage builds
