...
export default function DefenderRadar({ size = 'small' }) {
  const view = useMemo(() => nodes.map((node) => ({
    ...node,
    x: 160 + Math.cos((node.angle * Math.PI) / 180) * node.radius * 2.4,
    y: 160 + Math.sin((node.angle * Math.PI) / 180) * node.radius * 2.4,
  })), []);

  return (
    <div className={`radar ${size === 'large' ? 'radar-large' : ''}`}>
      <svg viewBox="0 0 320 320" role="img" aria-label="Defender threat horizon radar">
        <defs>
          <radialGradient id="radarFill"><stop offset="0%" stopColor="#54d8ff" stopOpacity=".14" /><stop offset="65%" stopColor="#54d8ff" stopOpacity=".04" /><stop offset="100%" stopColor="#54d8ff" stopOpacity="0" /></radialGradient>
      ...
      <div className="radar-center-label"><span>THREAT</span><small>HORIZON SIGNAL</small></div>
    </div>
  );
}
