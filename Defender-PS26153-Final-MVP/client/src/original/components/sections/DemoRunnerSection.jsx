...
const DemoRunnerSection = ({ onTabChange }) => {
  ...
  const runDemo = async (scenarioKey) => {
    ...
    try {
      ...
      setDemoResult({
        ...data,
        scenario_name: scenarioKey === 'normal' ? 'Network Telemetry Forecast' : 'Attack-Trajectory Forecast',
        risk_status: data.peak_risk > 0.7 ? 'HIGH RISK' : data.peak_risk > 0.5 ? 'ELEVATED RISK' : 'STABLE',
      });
    } catch (err) {
      setDemoResult(null);
      setErrorMsg(err.message || 'Forecast execution failed.');
    } finally {
      setIsRunning(false);
    }
  };
  ...
  return (
    <div style={{ width: '100%', maxWidth: '1280px', margin: '0 auto', padding: '40px 24px 100px' }}>
      ...
      {errorMsg && (
        <GlassCard style={{ padding: '16px 20px', marginBottom: '20px', border: '1px solid rgba(239, 68, 68, 0.45)' }}>
          <p style={{ margin: 0, color: '#FCA5A5', fontFamily: 'var(--font-body)', fontSize: '13px', lineHeight: 1.5 }}>
            Forecast unavailable: {errorMsg}
          </p>
        </GlassCard>
      )}
      ...
      {demoResult && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: '20px' }}>
            ...
            <GlassCard style={{ padding: '24px' }}>
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 12px' }}>
                Top Perturbation-Based Attributions
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
