...
import CyberChatLauncher from './components/cyber-chat/CyberChatLauncher';
import { CYPER_INITIAL_MESSAGES } from './components/cyber-chat/CyberChatSection';
import PrivacyPolicy from './pages/PrivacyPolicy';
import TermsOfService from './pages/TermsOfService';
import CookiePolicy from './pages/CookiePolicy';
import NotFound from './pages/NotFound';

const VALID_TABS = ['entry', 'home', 'demo', 'forecast', 'how_it_works', 'evidence', 'technical', 'privacy', 'terms', 'cookies'];
...
function App() {
  ...
  const [cyperContext, setCyperContext] = useState(null);
  const [cyperMessages, setCyperMessages] = useState(CYPER_INITIAL_MESSAGES);
  const [cyperGuideStep, setCyperGuideStep] = useState(null);
  ...
  return (
    <div className="defender-app" style={{ minHeight: '100vh', position: 'relative', backgroundColor: 'var(--bg-void)' }}>
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'forecast' && (
              <ForecastWorkspace autoRunDemo={false} onForecastResult={setCyperContext} />
            )}
            ...
            {!VALID_TABS.includes(activeTab) && <NotFound onTabChange={handleTabChange} />}
          </motion.div>
        </AnimatePresence>
      </main>
      ...
      {/* CYPER floating launcher -- persistent across every page, overlays
          rather than competing with the primary navigation/CTA hierarchy */}
      <CyberChatLauncher
        cyperContext={cyperContext}
        onTabChange={handleTabChange}
        messages={cyperMessages}
        onMessagesChange={setCyperMessages}
        guideStep={cyperGuideStep}
        onGuideStepChange={setCyperGuideStep}
      />
