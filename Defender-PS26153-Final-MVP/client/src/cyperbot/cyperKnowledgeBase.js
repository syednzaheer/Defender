...
export const KNOWLEDGE_BASE = [
  ...
  },
  {
    ...
    category: 'overview',
    keywords: ['why predictive', 'predictive defense', 'why forecast attacks', 'traditional ids', 'intrusion detection versus', 'classification versus forecasting', 'why does defender use temporal', 'temporal behaviour', 'temporal behavior'],
  ...
  },
  {
    id: 'sih_ps26153',
    category: 'sih',
    keywords: ['sih', 'ps 26153', 'ps26153', 'ntro', 'problem statement', 'national technical research organisation', 'sih 2026'],
    answer: '**SIH 2026 — PS 26153 (NTRO):** "AI based Network Attack Forecasting from Network Traffic Data". Organised for the National Technical Research Organisation. Defender solves this problem by representing network traffic as a sequence of canonical 22-dimensional states, learning state dynamics with a two-layer PyTorch LSTM, rolling predictions forward across K future time steps, mapping the progression to MITRE ATT&CK stages, and exposing perturbation-based feature attributions for analyst decision support.',
  },
  {
    id: 'networking_ipv4_ipv6',
    category: 'networking',
    keywords: ['ipv4', 'ipv6', 'ip address', 'ip addresses', 'difference between ipv4 and ipv6', 'dual stack'],
    answer: '**IPv4 vs IPv6:** IPv4 uses 32-bit addresses (~4.3 billion total), while IPv6 uses 128-bit addresses to solve IP exhaustion. IPv6 headers remove checksums to speed up routing and natively support IPSec. Defender flow ingestion extracts source and destination addresses to group traffic into bidirectional flows regardless of IP version.',
  },
  {
    id: 'networking_cidr_subnet',
    category: 'networking',
    keywords: ['cidr', 'subnet', 'subnetting', 'subnet mask', 'ip range', 'prefix length'],
    answer: '**CIDR & Subnetting:** Classless Inter-Domain Routing (CIDR) uses variable-length subnet masks (e.g. `/24` = 256 IPs) to partition IP address space efficiently. Subnetting isolates internal networks (east-west traffic) from boundary perimeter traffic (north-south traffic), which is critical when identifying lateral movement vs external reconnaissance.',
  },
  {
    id: 'networking_tcp_udp_icmp',
    category: 'networking',
    keywords: ['tcp vs udp', 'tcp and udp', 'icmp', 'three way handshake', '3-way handshake', 'syn ack ack', 'transport protocol'],
    answer: '**TCP vs UDP vs ICMP:**\n\n• **TCP (Protocol 6):** Connection-oriented and reliable. Uses a 3-way handshake (SYN → SYN-ACK → ACK), sequence numbers, windowing, and flow control.\n• **UDP (Protocol 17):** Connectionless and low-overhead. No handshake or retransmission; ideal for DNS or streaming.\n• **ICMP (Protocol 1):** Network control and error reporting (e.g., ping, unreachable messages).\n\nDefender records protocol_number as a key feature in its 22-dimensional state vector.',
  },
  {
    id: 'networking_ports',
    category: 'networking',
    keywords: ['port', 'ports', 'ephemeral port', 'destination port', 'source port', 'well known ports'],
    answer: '**Ports & Ephemeral Ports:** Ports identify specific services on a host. System/well-known ports range from 0–1023 (e.g., 80 HTTP, 443 HTTPS, 22 SSH, 53 DNS); dynamic/ephemeral ports (49152–65535) are assigned by the OS for client outbound connections. Defender uses source_port and destination_port to detect service targeting and asymmetric flow patterns.',
  },
  {
    id: 'networking_dns_dhcp_arp',
    category: 'networking',
    keywords: ['dns', 'dhcp', 'arp', 'mac address', 'domain name system', 'address resolution protocol'],
    answer: '**DNS, DHCP & ARP:**\n\n• **DNS (Port 53):** Resolves domain names to IP addresses. Tunneling or high-entropy subdomains can indicate C2.\n• **DHCP (Ports 67/68):** Dynamically assigns IP addresses and network configurations to hosts.\n• **ARP:** Maps IPv4 addresses to physical MAC addresses on local ethernet segments; ARP spoofing is a common local redirection vector.',
  },
  {
    id: 'networking_netflow_pcap',
    category: 'networking',
    keywords: ['netflow', 'ipfix', 'pcap', 'pcapng', 'netflow vs pcap', 'packet capture', 'flow telemetry'],
    answer: '**NetFlow / IPFIX vs PCAP:**\n\n• **NetFlow / IPFIX:** Summary statistics of flows (ports, IP addresses, bytes, packet count, duration, TCP flags). Lightweight and metadata-only.\n• **PCAP / PCAPNG:** Full packet captures containing exact raw headers (TTL, TCP window size, payload size, retransmissions).\n\nDefender’s 22-feature contract unifies 17 flow-level fields with 5 packet-level fields so both modalities can be processed in a single state vector.',
  },
  {
    id: 'networking_telemetry_metrics',
    category: 'networking',
    keywords: ['ttl', 'tcp window', 'retransmission', 'inter arrival time', 'iat', 'jitter', 'bandwidth', 'latency'],
    answer: '**Network Telemetry Metrics:**\n\n• **TTL (Time to Live):** Decremented per hop; reveals operating system families and routing changes.\n• **TCP Window Size:** Buffer space available for receiving data; indicates window scaling or transport throttling.\n• **Retransmissions:** Resent packets due to loss or congestion.\n• **IAT (Inter-Arrival Time):** Packet timing distribution (mean, variance, max); helps distinguish human interaction from scripted beaconing.',
  },
  {
    id: 'cybersecurity_ids_ips',
    category: 'security',
    keywords: ['ids vs ips', 'ids', 'ips', 'intrusion detection', 'intrusion prevention', 'difference between ids and ips'],
    answer: '**IDS vs IPS:** An **Intrusion Detection System (IDS)** passively monitors network traffic and alerts analysts upon detecting suspicious activity. An **Intrusion Prevention System (IPS)** sits inline and actively blocks or drops matching traffic. Defender acts as an AI decision-support forecasting layer above traditional IDS/IPS sensors to provide lead-time predictive awareness.',
  },
  {
    id: 'cybersecurity_detection_types',
    category: 'security',
    keywords: ['signature detection', 'anomaly detection', 'behavioral detection', 'signature versus anomaly', 'temporal detection'],
    answer: '**Detection Paradigms:**\n\n1. **Signature-Based:** Matches traffic against known static patterns (rules/hashes). High precision for known attacks, but blind to zero-days.\n2. **Anomaly-Based:** Compares traffic to a statistical baseline. Detects novel behavior but prone to high false positives.\n3. **Behavioral / Temporal (Defender):** Models state-transition dynamics $P(S_{t+1}|S_t)$ over sequence windows to forecast trajectory progression BEFORE an attack completes.',
  },
  {
    id: 'cybersecurity_metrics',
    category: 'security',
    keywords: ['false positive rate', 'fpr', 'precision', 'recall', 'f1 score', 'f1', 'false positives', 'false negatives'],
    answer: '**Evaluation Metrics & Trade-offs:**\n\n• **Precision:** Proportion of flagged flows that are genuinely malicious.\n• **Recall:** Proportion of total malicious flows successfully detected.\n• **F1 Score:** Harmonic mean of precision and recall.\n• **FPR (False Positive Rate):** Proportion of benign traffic incorrectly flagged as malicious.\n\nOn the CSE-CIC-IDS2018 cross-day split, Defender’s LSTM achieved **Recall: 0.6037** and **F1: 0.3492** (compared to Logistic Regression baseline Recall: 0.5744, F1: 0.3649), demonstrating progression awareness while highlighting real-world domain drift challenges.',
  },
  {
    id: 'cybersecurity_drift',
    category: 'security',
    keywords: ['concept drift', 'model drift', 'data drift', 'distribution drift', 'domain drift'],
    answer: '**Model Drift & Concept Drift:** Data drift occurs when baseline network traffic changes (e.g., new software deployments). Concept drift occurs when attacker techniques evolve. Defender addresses this by logging reliability metadata (novelty fraction, missing features, defer recommendations) so analysts know when traffic deviates from the model’s training distribution.',
  },
  {
    id: 'ml_lstm_forecasting',
    category: 'ml',
    keywords: ['why lstm', 'lstm architecture', 'hidden state', 'sequence modeling', 'recurrent neural network', 'sliding window', 'how does the lstm help', 'how does lstm help', 'how lstm helps'],
    answer: '**Why LSTM for Attack Forecasting:** Networks exhibit temporal dependency where prior states influence future risks. Standard classifiers evaluate each flow independently without memory. Defender uses a **2-layer PyTorch LSTM** (hidden size 64) with a sliding window of $W=10$ snapshots to encode historical trajectory and predict the next network state $Ŝ_{t+1}$.',
  },
  {
    id: 'ml_autoregressive_rollout',
    category: 'ml',
    keywords: ['autoregressive', 'k step rollout', 'rollout simulation', 'how k step works', 'recursive prediction'],
    answer: '**Autoregressive K-Step Rollout:** To forecast $K$ steps into the future, Defender predicts next state $Ŝ_{t+1}$ and hazard probability $P_1$. It then appends $Ŝ_{t+1}$ to the sequence window to predict $Ŝ_{t+2}$ and $P_2$, repeating for $K$ steps. This generates a full trajectory curve $[P_1, P_2, \dots, P_K]$ reflecting forecasted attack progression.',
  },
  {
    id: 'ml_dual_head_loss',
    category: 'ml',
    keywords: ['dual head', 'loss function', 'bce', 'mse', 'state head', 'hazard head'],
    answer: '**Dual-Head Architecture & Loss Function:** The LSTM features two output heads:\n1. **Linear State Head:** Predicts the 22-dimensional next feature state $Ŝ_{t+1}$ using Mean Squared Error ($L_{state}$).\n2. **Sigmoid Hazard Head:** Predicts infiltration probability using Binary Cross-Entropy ($L_{hazard}$).\n\nThe combined training objective is $L = L_{state} + \lambda L_{hazard}$, ensuring the model learns physical network dynamics alongside threat scoring.',
  },
  {
    id: 'ml_feature_attribution_vs_shap',
    category: 'ml',
    keywords: ['shap', 'shapley', 'why not shap', 'perturbation attribution', 'feature attribution vs shap', 'feature sensitivity'],
    answer: '**Perturbation Attribution vs SHAP:** Defender uses **perturbation-based feature attribution** (zeroing/perturbing specific features in the state window and measuring the resulting change in hazard output). It is NOT literal SHAP (Shapley Additive exPlanations), which requires expensive game-theoretic sampling across all feature coalitions. Perturbation attribution provides real-time, inspectable feature sensitivity for local analyst decision support.',
  },
  {
    id: 'datasets_classification',
    category: 'datasets',
    keywords: ['datasets supported', 'dataset list', 'cic-ids2017', 'cse-cic-ids2018', 'unsw-nb15', 'ctu-13', 'ciciot2023', 'lanl', 'darpa'],
    answer: '**Defender Dataset Registry & Support Status:**\n\n1. **VERIFIED / RUNNABLE:** `CSE-CIC-IDS2018` (Official AWS Open Data Wednesday-28-02-2018 train, Thursday-01-03-2018 test split).\n2. **ADAPTER-READY / COMPATIBLE:** `CIC-IDS2017`, `CICIoT2023`, `UNSW-NB15`, `CTU-13` (mapped via the canonical 22-feature schema adapter).\n3. **REFERENCE ONLY:** `LANL Authentication Dataset`, `DARPA IDS` (used for taxonomy and threat intelligence evaluation).',
  },
  {
    id: 'troubleshooting_csv_upload',
    category: 'troubleshooting',
    keywords: ['upload error', 'csv error', 'csv failed', 'why csv failed', 'invalid csv', 'file size limit', 'malformed csv'],
    answer: '**CSV Upload Troubleshooting:**\n\n• **File size:** Must be $\le 50$ MB.\n• **Header check:** Must contain standard flow feature columns (e.g. `Dst Port`, `Protocol`, `Flow Duration`, `Tot Fwd Pkts`, etc.).\n• **Format:** Must be UTF-8 encoded text with valid numerical flow values.\n• **Safety:** Uploaded files are validated, written temporarily to disk during scoring, and immediately deleted afterwards.',
  },
  {
