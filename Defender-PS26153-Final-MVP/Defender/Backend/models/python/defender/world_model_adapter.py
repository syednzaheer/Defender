...
from .integration_contract import FLOW_FEATURES, MODEL_FEATURES, canonicalise_pipeline_frame, feature_matrix
...
def _result_from_curve(
    ...
    stage_trajectory: Optional[list[str]] = None,
    row_stages: Optional[np.ndarray] = None,
) -> ForecastResult:
    canonical, _ = canonicalise_pipeline_frame(frame)
    if probabilities is None:
        probabilities = np.zeros(len(canonical), dtype=float)
    probabilities = np.clip(np.asarray(probabilities, dtype=float), 0.0, 1.0)
    stages = pd.Series(row_stages if row_stages is not None else (canonical.apply(_stage_for_row, axis=1) if len(canonical) else []), index=canonical.index, dtype=str)
    ...
    return ForecastResult(
        timeline,
        stage,
        explanations if explanations is not None else _explanations(canonical, stage),
        flagged,
        source,
        stage_trajectory or ([stage] * len(curve)),
    )


def forecast_with_world_model_artifact(frame: pd.DataFrame, model_path: str | Path, config_path: str | Path, steps: int = 5) -> ForecastResult:
    ...
    model.eval()
    windows = np.stack([values[start : start + window_size] for start in range(len(values) - window_size + 1)])
    with torch.no_grad():
        row_probabilities = []
        for start in range(0, len(windows), 2048):
            batch = torch.from_numpy(windows[start : start + 2048])
            _, batch_probability = model(batch)
            row_probabilities.extend(batch_probability.numpy().tolist())
    row_probabilities = np.asarray([0.0] * (window_size - 1) + row_probabilities, dtype=float)
    row_stages = np.asarray([_stage_for_row(pd.Series(row)) for row in values], dtype=object)
    ...
    with torch.no_grad():
        ...
        for _ in range(max(1, int(steps))):
            ...
            predicted_states.append(next_state.squeeze(0).numpy())
    ...
    for predicted_state in predicted_states:
        stage_trajectory.append(_stage_for_row(pd.Series(predicted_state, index=feature_names)))
    ...
    return _result_from_curve(
        ...
        explanations=explanation_table,
        stage_trajectory=stage_trajectory,
        row_stages=row_stages,
    )


forecast_with_trained_artifact = forecast_with_world_model_artifact
