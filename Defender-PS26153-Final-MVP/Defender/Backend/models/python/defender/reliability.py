...
from .integration_contract import MODEL_FEATURES, canonicalise_pipeline_frame
...
def assess_input(frame: pd.DataFrame, scaler_mean=None, scaler_scale=None) -> dict:
    ...
    """
    canonical, metadata = canonicalise_pipeline_frame(frame)
    values = canonical.loc[:, list(MODEL_FEATURES)].to_numpy(dtype=float)
    packet_features = ("ttl_mean", "ttl_variance", "tcp_window_mean", "payload_size_mean", "retransmission_count")
    missing_packet_features = list(frame.attrs.get("missing_packet_features", metadata.unavailable_features))
    result = {
        "rows": int(len(canonical)),
        "packet_features_available": [name for name in packet_features if name not in missing_packet_features],
        "packet_features_missing": missing_packet_features,
        "novelty_fraction": 0.0,
        "defer_recommended": bool(missing_packet_features),
    }
