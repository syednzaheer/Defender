...
3. **Dashboard & Analyst Interface Layer**:
   ...
   - **CYPER** local analyst guide mounted globally in `src/App.jsx`; answers from `src/data/cyperKnowledgeBase.js` and maps live `/api/v1/forecast` (or local fallback) results published by `ForecastWorkspace`.
   - Located in `src/components/`, `src/data/cyper*.js`, `server/`, and `src/defender/forecasting.py`.
