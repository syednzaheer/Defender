...
│   │       ├── CyperAssistantPanel.jsx  # Global CYPER analyst chat
│   │       └── cyperAssistant.css  # CYPER panel/launcher styles
│   ├── data/
│   │   ├── telemetryConstants.js   # Shared SHAP attribution rankings
│   │   ├── cyperKnowledgeBase.js   # Offline CYPER answers mapped to MVP facts
│   │   ├── cyperIntentMatcher.js   # Phrase matching for judge questions
│   │   ├── cyperForecastContext.js # Live forecast → CYPER context mapping
│   │   └── cyper.test.js           # CYPER intent and context regression suite
...
CYPER (global, all screens including Entry Portal)
    ├── Offline knowledge: dynamics, 22-feature contract, ATT&CK stages, judge path
    └── Live map: latest ForecastWorkspace result (stage, risk, timeline, attributions)
```
